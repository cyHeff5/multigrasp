...
def _grid_for_shape(shape: str, cfg: dict, step_cm: float) -> list[dict]:
    ...
    if shape == "rect_cylinder":
        # 2D-Raster: thickness × width. thickness > width wird ausgelassen —
        # der Sampler erzwingt thickness = kürzere Seite, solche Kombinationen
        # kämen im Training nie vor (waeren dasselbe Objekt, falsch gegriffen).
